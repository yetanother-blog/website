...
export type BioQueryQuery = { avatar?: Maybe<{ childImageSharp?: Maybe<{ fixed?: Maybe<GatsbyImageSharpFixedFragment> }> }>, site?: Maybe<{ siteMetadata?: Maybe<(
...
export type AllRepoBlogPostsQuery = { allMdx: { nodes: Array<(
      Pick<Mdx, 'excerpt' | 'timeToRead'>
      & { wordCount?: Maybe<Pick<MdxWordCount, 'words'>>, frontmatter?: Maybe<(
        Pick<MdxFrontmatter, 'title' | 'slug' | 'date' | 'format' | 'draft'>
        & { dateUrl: MdxFrontmatter['date'] }
      )> }
    )> } };
...
export type BlogPostBySlugQuery = { mdx?: Maybe<(
    ...
    & { wordCount?: Maybe<Pick<MdxWordCount, 'words'>>, frontmatter?: Maybe<Pick<MdxFrontmatter, 'title' | 'date'>> }
  )> };
