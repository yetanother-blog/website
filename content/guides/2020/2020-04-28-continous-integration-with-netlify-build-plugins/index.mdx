...
Netlify is a great tool to easily deploy and host your static website without any hazzle
but when it comes to a real-life continuous integration it can become a bit tricky. For this blog,
we used a GitHub workflow to run unit tests, lint the code and check the _yarn.lock_ file for uncomitted changes.
...
That’s it! Our CI pipeline written in pure JavaScript! Of course, it’s a simple example but already enough
for us to validate our application. Netlify passes some input to our plugin we can use to interact
