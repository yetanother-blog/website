...
Okay, so let me get this straight: This blog is deployed on Netlify because we love Netlify ❤️. Seriously, they put just a nice focus on everything you need to be a happy developer: It’s well integrated with GitHub, you get fancy preview URLs for every single pull request, you can run your website with zero configuration and beyond that they offer a great Netlify CLI and API in case you want to hack their system. So everything is great? Almost! The thing is, I don’t work on this blog alone and so we needed a continuous integration (CI) pipeline to keep our blog running smoothly. We do boring stuff like running unit tests, linting the code or check the `yarn.lock` file for uncommitted changes. As I said, boring stuff but really important. In order to get this pipeline we configured a GitHub workflow to run all the checks for every pull request. We, of course, also run the workflow with every merge on master so we ended up disabling Netlify’s great **auto publishing** feature and used a GitHub action to deploy the blog manually within the workflow. Sure it works, but it feels messy and I was super unhappy to loose the full power of the Netlify ecosystem.

## So, what are Netlify Build Plugins?

It’s such a hot topic! They just released the beta version and you can enable it for your website right in the Netlify app. With that, you can basically extend the build pipeline and do whatever you want (well more or less).
...
I can’t wait to see more awesome plugins written by the open source community, but besides that, we can also use it to define our own flow. So coming back to the CI pipeline I mentioned earlier.
...
What do you think? It’s still beta and I have stuff on my wish list. I would love to see running plugins in parallel so we could speed up our build pipeline. Another thing would be to have GitHub pull request checks on a plugins bases so I can easily identify the failing plugins within my GitHub workflow and probably get the error directly in GitHub so I don’t need to scroll through a horrible debug log. But overall, I guess it’s a great step in the right direction to stay in the Netlify ecosystem and don’t mess up different tools to get things done like a simple CI pipeline or notifying you on Slack.
...
- [GitHub - netlify/build: Netlify Build Plugins. Join the early access beta 👉](https://github.com/netlify/build)
